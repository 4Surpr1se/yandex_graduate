...
def test_create_film_price():
    ...
    response = requests.post(f"{BASE_URL}/film_prices", json=request_data)
...
def test_calculate_film_price(film_price_data, tax_rate_data, discount_data):
    ...
    response = requests.post(f"{BASE_URL}/film_prices", json=film_price_data)
    assert response.status_code == 201

    # добавление налога и скидки
    response = requests.post(f"{BASE_URL}/tax_rates", json=tax_rate_data)
    assert response.status_code == 201
    ...
    response = requests.post(f"{BASE_URL}/calculate_film_price", json=request_data)
...
def test_calculate_subscription_price(subscription_data, tax_rate_data):
    ...
    response = requests.post(f"{BASE_URL}/calculate_subscription_price", json=subscription_data)
    assert response.status_code == 200
    result = response.json()
