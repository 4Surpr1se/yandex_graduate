...
@backoff()
def get_updated_genres_ids(cursor: PsycopgCursor, last_modified_date: datetime, batch_size: int):
    try:
        logger.info("Try get_updated_genres_ids")
        query = """
                SELECT id, updated_at FROM content.genre 
                WHERE updated_at > %s 
                ORDER BY updated_at ASC 
                LIMIT %s
        """
